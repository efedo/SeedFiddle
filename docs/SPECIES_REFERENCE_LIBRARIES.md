...
Seed Fiddle will be able to turn deliberately selected, reviewed annotations
...
The first release will reuse the application's existing, interpretable
classifiers. It will not introduce a server or a neural training pipeline.
...
10. **One project pins one library version per species.** There is no silent
    fallback to a newer version, and a missing or incompatible pin is a visible
    degraded/error state.

## Scope of the first usable release

The first release will support these species-library products:
...
| Shape summary | Complete annotated seed masks | Seed scale estimate and later procedural priors | Audited distributions only at first; no automatic procedural constraint until separately validated |
...
```text
reviewed image + v3 reference sidecar + species vocabulary
                    |
...
The proposed node outputs compact typed data, not full-resolution rasters:
...
Each consumer that can use reusable references gets an honest `Reference
source` control with three values:
...
`manifest.json` will contain at least:
...
mixture component, and prototype therefore carries a source index and, where
applicable, a seed ID. Runtime assembly filters the current source SHA-256 and
then applies deterministic quotas.
...
example camera/session/lot). Invalid sources are reported individually and are
never silently skipped during publishing.
...
6. `Seed scale estimate` initially displays the library width/area distribution
   only as an additional diagnostic. Using it as a prior requires a later,
...
Version-one projects load unchanged with no pin. Saving them writes version two.
Reference-region archives remain version three; no mask migration is required.
...
2. **Sources:** eligible project images, annotation state, species, fingerprint,
   capture group, included products, and validation warnings.
...
active pin and whether the current image occurs in it. On an image that is a
library source, the panel must say that its contribution was excluded.
...
- Factor shared colour, noise, material, edge, trait, and shape descriptor seams.
...
- Display library maximum-Feret width, area, aspect, solidity, and concavity
  distributions in `Seed scale estimate` diagnostics.
...
not treated as negatives; shape statistics exclude cutoff/incomplete seeds.
